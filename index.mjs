...
const getWorkScheduleResultExport = async ({
  results,
  connection,
  queryDatabase,
}) => {
...
export const handler = async (event) => {
  ...
  async function uploadWorkSchedules() {
    ...
    listUnique.forEach((row) => {
      ...
      row.appliedDate = row.appliedDate
        ...
        : null;
      row.accepted = row.accepted || row.deletedStatus ? "対応済み" : "未対応";
      row.intType = getIntroductionHistoryTypeJP(row.intType);
      row.acceptedReason = row.acceptedReason
        ? row.acceptedReason.replace(/\n/g, " ")
        : "";
  ...
  console.log('1) Start upload 確定シフトCSVダウンロード');
  await uploadAcceptedWorkSchedules();
  console.log('Finished upload 確定シフトCSVダウンロード');

  console.log('2) Start upload 未充足シフトCSVダウンロード');
  await uploadSatisfieldshifts();
  console.log('Finished upload 未充足シフトCSVダウンロード');

  console.log('3) Start upload 半未充足シフトCSVダウンロード');
  await uploadUnfulfilledshifts();
  console.log('Finished upload 半未充足シフトCSVダウンロード');

  console.log('4) Start upload 募集シフトCSVダウンロード');
  await uploadAvailableshifts();
  console.log('Finished upload 募集シフトCSVダウンロード');

  console.log('5) Start upload 応募シフトCSVダウンロード');
  await uploadWorkSchedules();
  console.log('Finished upload 応募シフトCSVダウンロード');

  console.log('6) Start upload 医師情報CSVダウンロード');
  await uploadDoctors();
  console.log('Finished upload 医師情報CSVダウンロード');

  console.log('7) Start upload お問い合わせ管理のチャッ');
  await uploadConversations();
  console.log('Finished upload お問い合わせ管理のチャッ');
